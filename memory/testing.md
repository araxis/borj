...
- **Acceptance metrics:** wait for `qa-state-recorded` before visual assertions. Dragon closeup should report
  ...
  including `idle`, `walk`, and `attack`, and `actorProfile: 'animated-crawler'`. With the local animated worm repair,
...
## Updated 2026-06-30 - animated worm asset repair QA
- **Implementation baseline:** `public/assets/animals/Worm.glb` is locally repaired as an animated GLB generated by
...
- **Static checks:** for memory-only packaging, `git diff --check` is enough. If asset/source files are touched again,
