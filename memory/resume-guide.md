...
- Actor quality gate: run `npm run audit:assets` before accepting new visible actor GLBs. Production actor assets should
  ...
  `Worm.glb` are both animation-ready with idle/walk/attack clips. Do not add static-GLB bob/weave animation branches
  for primary actors.
...
- **Current boss actor baseline:** Runtime Boss Actor Quality is merged on `main` through PR #13, Animated Worm Asset
  Repair is merged on `main` through PR #15, and Animated Dragon Asset Repair is merged on `main` through PR #17.
  Dragon closeup should still use animated `a_azhdaha_actor` with
...
- **Animated dragon asset baseline:** `a_dragon` is now animation-ready fallback/source material. `Azhdaha.glb` is
  generated by `scripts/asset-tools/build-animated-dragon.mjs`, contains `Idle`, `Walking`, and `Attack`, and should no
  longer appear in `sourceOnly`. Static site deploy run `28492906102` completed successfully for PR #17 merge commit
  `f96924c`.
...
- **Recommended next implementation step:** treat Harmony, Combined Combat Readability, Runtime Boss Actor Quality,
  Animated Worm Asset Repair, and Animated Dragon Asset Repair as the shipped `main` baseline. Do not start another
  asset or gameplay pass unless the user explicitly requests one.
