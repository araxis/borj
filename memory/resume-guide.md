...
- **Current boss actor baseline:** Runtime Boss Actor Quality is merged on `main` through PR #13, Animated Worm Asset
  Repair is merged on `main` through PR #15, and local Animated Dragon Asset Repair is verified on top of them. Dragon
  closeup should still use animated `a_azhdaha_actor` with
...
- **Recommended next implementation step:** treat Harmony, Combined Combat Readability, Runtime Boss Actor Quality, and
  Animated Worm Asset Repair as the shipped `main` baseline, with local Animated Dragon Asset Repair ready for
  validation, commit, PR, and merge. After that merge, the next step should be post-merge closeout memory only unless a
  separate new asset or gameplay pass is explicitly requested.
