...
const FRAG = `
  ...
  void main() {
    ...
    rgb = mix(rgb, rgb * tint, 0.35);
    float lowFog = (1.0 - smoothstep(0.14, 0.92, vUv.y)) * 0.26;
    rgb = mix(rgb, fogTint, lowFog);
    rgb = clamp(rgb * brightness, 0.0, 1.0);
    float edge = smoothstep(0.0, edgeFade, vUv.x) * (1.0 - smoothstep(1.0 - edgeFade, 1.0, vUv.x));
...
function prepBackdropTexture(tex) {
...
function loadLayerTexture(url, root, mesh, mat, entry) {
  if (!url) {
...
function rangeFor(biomeKey) {
  const table = {
    highland: { color: 0x7f8b6c, opacity: 0.22, peak: 15, R: 282, layers: 2 },
    valley: { color: 0x71875f, opacity: 0.2, peak: 13, R: 286, layers: 2 },
    plains: { color: 0x92946a, opacity: 0.18, peak: 10, R: 292, layers: 1 },
    forest: { color: 0x496a58, opacity: 0.2, peak: 17, R: 274, layers: 2 },
    mountain: { color: 0x7e8490, opacity: 0.26, peak: 22, R: 268, layers: 2 },
    snowpeak: { color: 0xb6c5d4, opacity: 0.26, peak: 24, R: 266, layers: 2 },
    steppe: { color: 0x938b68, opacity: 0.28, peak: 16, R: 275, layers: 2 },
    desert: { color: 0xb99661, opacity: 0.3, peak: 14, R: 260, layers: 2 },
    river: { color: 0x738966, opacity: 0.18, peak: 12, R: 288, layers: 1 },
    wetland: { color: 0x57705f, opacity: 0.16, peak: 10, R: 292, layers: 1 },
  };
...
const RING = { N: 18, tileW: 135, baseY: -14, peakBoost: 1.0 };
...
function fillProceduralRing(ringGroup, cfg) {
  ...
  for (let layer = 0; layer < layers; layer++) {
    ...
    for (let i = 0; i < RING.N; i++) {
      ...
      const R = layerCfg.R * (0.92 + ringRand(i + layer * 7, 4) * 0.18);
